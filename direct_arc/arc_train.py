...
def arc_train(train_corpus, train_type, num_words, num_labels, modelname, hidden_size = 200, lr = 0.005, dropout = 0.25, num_epochs = 3, num_layers = 2, batch_size = 1, model = 'LSTM1', lm = None):
	...
	for epoch in range(num_epochs):
		...
		for i in range(1, len(train_corpus), batch_size):
			...
			#Run LSTM
			# outputs = classifier.forward(word_batch)
			#Run LM
	...
	save_path = os.path.join('/storage/vsub851/864_final_project/checkpoints', modelname)
...
def test_train(base_path, model_name, filename = 'en_ewt-ud-train.conllu', lm = None):
	...
	arc_train(train_corpus, train_type = 'heads', num_words = num_words, num_labels = num_labels, modelname = model_name, hidden_size = hidden_size, lr = lr, dropout = dropout, num_epochs = num_epochs, 
			num_layers = num_layers, batch_size = batch_size, model = model, lm = lm)

# test_train(base_path = base_path, model_name = 'lm2head.pt', lm = lm_pretrained)